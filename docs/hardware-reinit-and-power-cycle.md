...
| `boardUseCanTerminator` (board-specific field of hellen-honda-k / hellen-112-17, listed in their `board_constants_extensions.ini`) | Applied by `configureHellenCanTerminator()` which is explicitly guarded by a `static bool initialized` - the terminator output is written exactly once (`config/boards/hellen/hellen_common.cpp`). |
