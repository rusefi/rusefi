...
## The Solution: `applyDefaultsOrFixAfterBurn()`
The `applyDefaultsOrFixAfterBurn()` function in `firmware/controllers/algo/defaults/default_base_engine.cpp` is specifically designed to handle these migrations. This function is called during startup and whenever a "burn" (save configuration) operation occurs.
...
## When to use `applyDefaultsOrFixAfterBurn()` vs `setDefaultBaseEngine()`
...
- **`applyDefaultsOrFixAfterBurn()`**: Use this for *migration* logic. It ensures that *existing* configurations get reasonable values for newly added fields that they don't yet have values for.

## Best Practices
1. **Choose 0 carefully**: If possible, design new parameters so that 0 is a safe/neutral value. If 0 must have a specific meaning, consider if a different "uninitialized" value is needed or if `applyDefaultsOrFixAfterBurn()` migration is required.
2. **Avoid breaking changes**: If you must change the meaning of an existing field, you may need more complex migration logic in `applyDefaultsOrFixAfterBurn()` to translate old values to new ones.
