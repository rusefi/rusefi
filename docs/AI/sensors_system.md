...
- **TPS**: the closest existing machinery is `TpsAccelEnrichment` (`firmware/controllers/algo/accel_enrichment.cpp`). It receives `Tps1` from `Engine::periodicSlowCallback()` (20 Hz) via `onNewValue()`, keeps a `cyclic_buffer<float>` history, and publishes the largest adjacent-sample delta as `tpsFrom`/`tpsTo`/`deltaTps` plus `isAboveAccelThreshold`/`isBelowDecelThreshold` (see `tps_accel_state.txt`). Caveats: the delta is *per sample step, not per second* (units depend on callback rate and buffer length), and it is hardwired to TPS1 and fuel-AE semantics.
