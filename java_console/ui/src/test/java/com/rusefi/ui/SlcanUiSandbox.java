...
/**
 * Manual Swing sandbox around {@link SlcanClient}: scans serial ports for the rusEFI CAN
 * sniffer SLCAN channel (skipping the TunerStudio console port) and keeps reconnecting.
 * <p>
 * Top-right square shows connection state: green = SLCAN port located and connected,
 * red = scanning/disconnected. Most of the window shows the last {@link #VISIBLE_FRAMES}
...
public class SlcanUiSandbox {
    ...
    public SlcanUiSandbox() {
        ...
        frame.getContentPane().add(new JScrollPane(framesArea), BorderLayout.CENTER);
        ...
        Thread reader = new Thread(this::readerLoop, "SLCAN UI reader");
    ...
    private void readerLoop() {
        while (true) {
            SlcanClient client = SlcanClient.findAndConnect(System.out::println);
            if (client == null) {
                try {
            ...
            try {
                ...
                System.out.println(client.getPort() + ": disconnected: " + e);
            } finally {
                connectedPort = null;
                client.close();
            }
    ...
    private void saveBuffer(JFrame parent) {
        List<FrameRecord> snapshot;
        synchronized (lock) {
            snapshot = new ArrayList<>(buffer);
        }
        if (snapshot.isEmpty()) {
            JOptionPane.showMessageDialog(parent, "Buffer is empty, nothing to save", "SLCAN", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        ...
        try (PrintWriter writer = new PrintWriter(Files.newBufferedWriter(file.toPath(), StandardCharsets.US_ASCII))) {
            ...
            JOptionPane.showMessageDialog(parent, "Failed to save: " + e, "SLCAN", JOptionPane.ERROR_MESSAGE);
            return;
        }
        JOptionPane.showMessageDialog(parent, "Saved " + snapshot.size() + " frame(s) to " + file.getAbsolutePath(),
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(SlcanUiSandbox::new);
    }
}
