...
# Note that flock doesn't ship on macOS by default. Install it with 'brew install flock';
# without it the gradle invocations run unlocked (gradle's own daemon locking serializes them).
# On Windows, flock comes with Cygwin.
FLOCK_BIN := $(shell command -v flock 2>/dev/null)
ifeq ($(FLOCK_BIN),)
FLOCK = true
else
