...
$(CONFIG_DEFINITION_JAR): .FORCE
	cd $(JAVA_ROOT) && $(FLOCK) ./gradlew :config_definition:shadowJar

$(CONFIG_DEFINITION_BASE_JAR): .FORCE
	cd $(JAVA_ROOT) && $(FLOCK) ./gradlew :config_definition_base:shadowJar

$(ENUM_TO_STRING_JAR): .FORCE
	cd $(JAVA_ROOT) && $(FLOCK) ./gradlew :enum_to_string:shadowJar

#$(TUNE_TOOLS_JAR):
	#cd $(JAVA_ROOT) && $(FLOCK) ./gradlew :tune-tools:shadowJar

$(TS_PLUGIN_LAUNCHER_JAR): .FORCE
	cd $(JAVA_ROOT) && $(FLOCK) ./gradlew :ts_plugin_launcher:shadowJar

$(CONSOLE_JAR): .FORCE
	cd $(JAVA_ROOT) && $(FLOCK) ./gradlew :ui:shadowJar

$(AUTOUPDATE_JAR): .FORCE
	cd $(JAVA_ROOT) && $(FLOCK) ./gradlew :autoupdate:shadowJar
