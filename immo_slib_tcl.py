...
# Breakpoints v3:
#
# Выводы из двух провальных сессий:
#  - 0x08069028/0x080697D0: никогда не вызываются без диагностики (OBD)
#  - 0x08202038/0x0820743A/0x082027A4/0x08201E2C: не вызываются без BCM кадров
#  - Причина: ЭБУ триггерится на 0x066A (byte0=0x08), НЕ на 0x0350!
#
# Данные из stol.trc (единственный трейс где 0x0713 реально отправился):
#   За 510мс до 0x0713 изменились: 0x066A→08FF... 0x0648→C3FF... 0x041A→0003...
#   0x0350 вообще не было в трейсе.
#
# СТРАТЕГИЯ:
#  BP1: 0x08201000 — ТОЧНО срабатывает при любом reset (проверено ранее)
#       Если не срабатывает — проблема с SWD/питанием.
#  BP2: 0x08201710 — BLX R3 в wrapper (вызывается при init, проверено шагами)
#  BP3: 0x082017AA — BLX R3 рядом (второй вызов в той же цепочке)
#  BP4: 0x082027A4 — вычислитель ответа (нужен 0x0714 от BCM)
# ─────────────────────────────────────────────────────────────────────────────
BREAKPOINTS = [
    (0x08201000, 'IMMO wrapper entry — ВСЕГДА срабатывает при reset (диагностика)'),
    (0x08201710, 'BLX R3 в wrapper init-цепочке (шаги подтвердили выполнение)'),
    (0x082017AA, 'BLX R3 в wrapper (второй вызов той же функции)'),
    (0x082027A4, 'FUN_082027A4 — вычислитель ответа (нужен 0x0714 от BCM)'),
]
...
ТРЕБОВАНИЯ ДЛЯ ЗАПУСКА:
  ...
  2. PCAN шлёт BCM-кадры (данные из stol.trc, что реально триггерят 0x0713):

     УБЕРИ 0x0350 - он НЕ нужен для оригинального ЭБУ!

     Добавь в PCAN циклично (IDFormat=hex):
       ID=66A  DLC=8  Data=08FF000101C00000  CycleTime=100
       ID=648  DLC=8  Data=C3FFF000007FFE18  CycleTime=100
       ID=41A  DLC=8  Data=0003000000000000  CycleTime=100
       ID=5DA  DLC=8  Data=41840000BAFFFF00  CycleTime=100
       ID=36E  DLC=4  Data=09990999          CycleTime=100
       ID=6FD  DLC=4  Data=00009920          CycleTime=100
       ID=41D  DLC=4  Data=48FEFF00          CycleTime=100

     Это точные данные CAN-шины из stol.trc за 510мс до отправки 0x0713.

  3. BP 0x08201000 = ДИАГНОСТИКА: должен сработать СРАЗУ после reset.
     Если не срабатывает - проблема с питанием или SWD.
""", 'y')
