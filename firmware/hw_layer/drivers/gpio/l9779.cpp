...
int L9779::init()
{
	...
	/* The AT32 SPI HAL toggles chip select but does not configure its pad.
	 * Latch the inactive level before enabling the output to avoid a low
	 * pulse while the pin changes mode. */
	...
	/* Keep the realtime counter running for SPI delays on AT32 port variants
	 * that do not enable it during kernel startup. Do not reset a shared clock. */
	CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;
	DWT->CTRL |= DWT_CTRL_CYCCNTENA_Msk;

	/* The first pipelined reply predates SW_RST and can contain invalid idle
	 * data. Do not reject startup on that reply; subsequent reads validate the
	 * link, and KEY_ON must be read successfully before the board enables power. */
	(void)chip_reset();
