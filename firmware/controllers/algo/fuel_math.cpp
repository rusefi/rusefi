...
floatms_t getInjectorLag(float vBatt DECLARE_ENGINE_PARAMETER_SUFFIX) {
	...
	float vBattCorrection = interpolate2d_DEPRECATED("lag", vBatt, INJECTOR_LAG_CURVE);
...
float getCltFuelCorrection(DECLARE_ENGINE_PARAMETER_SIGNATURE) {
	...
	return interpolate2d_DEPRECATED("cltf", engine->sensors.clt, WARMUP_CLT_EXTRA_FUEL_CURVE) / PERCENT_MULT;
}

angle_t getCltTimingCorrection(DECLARE_ENGINE_PARAMETER_SIGNATURE) {
	if (cisnan(engine->sensors.clt))
		return 0; // this error should be already reported somewhere else, let's just handle it
	return interpolate2d_DEPRECATED("timc", engine->sensors.clt, engineConfiguration->cltTimingBins, engineConfiguration->cltTimingExtra, CLT_TIMING_CURVE_SIZE);
}

float getIatFuelCorrection(float iat DECLARE_ENGINE_PARAMETER_SUFFIX) {
	if (cisnan(iat))
		return 1; // this error should be already reported somewhere else, let's just handle it
	return interpolate2d_DEPRECATED("iatc", iat, IAT_FUEL_CORRECTION_CURVE);
}
...
floatms_t getCrankingFuel3(float coolantTemperature,
	...
	float durationCoef = interpolate2d_DEPRECATED("crank", revolutionCounterSinceStart, config->crankingCycleBins,
			config->crankingCycleCoef, CRANKING_CURVE_SIZE);

	float coolantTempCoef = interpolate2d_DEPRECATED("crank", coolantTemperature, config->crankingFuelBins,
			config->crankingFuelCoef, CRANKING_CURVE_SIZE);

	float tpsCoef = interpolate2d_DEPRECATED("crank", getTPS(PASS_ENGINE_PARAMETER_SIGNATURE), engineConfiguration->crankingTpsBins,
