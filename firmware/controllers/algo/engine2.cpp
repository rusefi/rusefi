...
void EngineState::updateSparkSkip() {
		...
		/*
		 * Is the following code in use? In C++ binary "+" operator has higher priority higher then ternary "?:"
		 * operator. It means that below we check if (luaSoftSparkSkip + engineConfiguration->useHardSkipInTraction)
		 * expression (sum of float and bool values) is zero or not. It's pretty weird :(
		 */
