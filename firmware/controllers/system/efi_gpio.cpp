...
OutputPin* OutputPin::s_firstOutput = nullptr;
...
void OutputPin::stopConfigurationList() {
	...
	while (listItem != nullptr) {
		if (listItem->brainPin != *(listItem->brainPinPtr)) {
			listItem->unregisterOutput(listItem->brainPin);
		}
...
void initPrimaryPins(Logging *sharedLogger) {
	logger = sharedLogger;
#if EFI_PROD_CODE
	enginePins.errorLedPin.initPinR("led: CRITICAL status", LED_CRITICAL_ERROR_BRAIN_PIN, &(LED_ERROR_BRAIN_PIN_MODE));
