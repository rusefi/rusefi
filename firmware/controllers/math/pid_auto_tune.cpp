...
bool PID_AutoTune::Runtime(Logging *logger)
{
  ...
  this->absMin = absMin;
    ...
    if (controlType == AMIGOF_PI)
    {
      ...
      // check that phase lag is within acceptable bounds, ideally between 120 and 140 deg
      // but 115 to 145 deg will just about do, and might converge quicker
      if (abs(phaseLag - CONST_PI * 130.0 / 180.0) > (CONST_PI * 15.0 / 180.0))
      {
        // phase lag outside the desired range
        // set noiseBand to new estimate
        // aiming for 135 deg = 0.75 * pi (radians)
        // sin(135 deg) = sqrt(2)/2
