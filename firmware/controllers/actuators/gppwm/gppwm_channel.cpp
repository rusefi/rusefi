...
void GppwmChannel::setOutput(float result) {
	...
	if (m_usePwm) {
		...
		m_pin->setValue(m_state);
	}
}

void GppwmChannel::init(bool usePwm, OutputPin* pin, SimplePwm* pwm, const ValueProvider3D* table, const gppwm_channel* config) {
	m_usePwm = usePwm;
	m_pin = pin;
	m_pwm = pwm;
	m_table = table;
