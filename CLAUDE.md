...
### isIgnVoltage() on m74_9 and the ignition-gated power-stage plan (analysis 2026-08-26, not implemented)
...
- L9779 power-stage gate plan: PSOFF = CONFIG_REG6 bit 4 (write 0x16) kills the power stages but keeps chip logic + regulators + SPI + WDA + KEY_ON alive (MCU keeps running). Today `chip_init()` runs at power-on with `L9779_CONFIG6_PWR=0x06` (PSOFF=0 - stages ON) before ignition. Planned: ignition off -> no `chip_init` / REG6=0x16 + no WDA feed + slow KEY_ON poll; key rising -> `need_init = true` -> existing `chip_reset()`+`chip_init()`+`update_output()`+WDA kick path; key off after running -> REG6=0x16 + stop WDA. TLE9201 (PB13/ETC_EN, driven high at board init) is NOT ignition-gated yet - separate.
