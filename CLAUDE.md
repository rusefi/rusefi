...
- **CORRECTION 2026-08-24: of the stock ramp's three registers, ONLY REG5 (0x05) is VRS. REG4 (0x04) is power management (PWL_TIMEOUT_CONF/ISO_SRC/LOCK), REG6 (0x06) is power management + WDA time base (PWL_EN_N, PSOFF, VDD5_UV RST/WDA masks, CONFIG6 bit1 = f_clk 64/39 kHz).** The stock's REG6 steps 0x07->0x05->0x05->0x06 FLIP THE WDA TIME BASE to 39 kHz at 150..600 rpm (values 0x05 have bit1=0): response time becomes (1+101*10)/39kHz = 25.9 ms, window [25.9, 38.5] ms, while the feed is clamped to 27 ms -> misses -> EC climb -> WDA kill pulses exactly during the catch (matches the miss=5..8 right at the catch in the 16:02/16:09 logs). Fixed: the ramp writes REG4/REG5/REG6 (full stock register traffic) with CONFIG6 bit1 PINNED to 1 (64 kHz) - values 0x07/0x07/0x07/0x06 - and `L9779_CONFIG6_PWR` = 0x06 is applied at init + in chip_heal_out_dis(true). Removing the REG4/REG6 writes entirely (tried 2026-08-24 evening) cost the car one tooth per revolution at ~2000 rpm (C9003 57/58) - the stock writes those registers for a reason, keep the traffic. CONFIG6 bit2 (VDD5_UV WDA mask) MUST stay 1: unmasked, a VDD5 undervoltage pulls WDA low and kills the blade on every cranking rail dip.
