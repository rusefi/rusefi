...
### Changing the composite layout strands Windows' descriptor cache — bump `bcdDevice`

Windows keys its cached USB descriptor and driver-binding state on **`idVendor` + `idProduct` + `bcdDevice`** (registry `HKLM\SYSTEM\CurrentControlSet\Control\usbflags\<VID><PID><bcdDevice>`). rusEFI's device descriptor in `usbcfg.cpp` is `0483:5740` with `bcdDevice = 0x0200`, while `NUM_INTERFACES` and `DESCRIPTOR_SIZE` right above it are *computed* from `HAL_USE_USB_MSD` / `HAL_USE_USB_CDC_2..4`. So enabling a second VCP changes the interface count, endpoint allocation and `wTotalLength` — but **not** the cache key. Every host that already enumerated the old firmware keeps applying the old layout, and the new/changed interfaces never bind: the VCP silently fails to appear. The tell is that a machine which has *never* seen the board works fine, which makes it look like a board fault rather than a host-cache fault. **Bump `bcdDevice` in the same change as any composite-layout change** — that is the only thing that invalidates the cache for users in the field; treat it like the OpenBLT `BLxx` marker.

Host-side cleanup for machines that already cached a stale layout: `misc/windows/usb-repair/fix-vcp.ps1` (dry run by default; `-Execute -All` from an elevated shell). It clears the three stores that hold stale state — ghost device instances (`Present = False`), the `usbflags` entry, and the COM Name Arbiter `ComDB` bitmap, which ghost boards fill from COM3 upward so each new board lands on a higher number. Two confusables it also reports: a board bound to **usbipd-win** shows as *USBIP Shared Device* on the `VBoxUSB` stub driver and produces **no COM port at all** until `usbipd unbind`; and the MSD interface (`MI_00`) can end up holding a `PortName` reservation despite not being a serial port.

PowerShell 5.1 traps when writing such tooling: `$PID` is a read-only automatic variable (assignment is a hard error), and `2>&1` on a native exe wraps each stderr line in a `NativeCommandError` `ErrorRecord` that `$ErrorActionPreference = 'Stop'` escalates to a terminating error *even on exit 0* — `usbipd` emits filter-driver warnings on every call, so this aborts scripts that look correct.
