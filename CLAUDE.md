...
Proposed fix (NOT implemented): convert the queued angle events to time ONE TOOTH EARLIER (lead 2 teeth = 333 us @6000). The prediction error over 12 deg is negligible: `oneDegreeUs` is a 90-deg moving average (`InstantRpmCalculator`), first-order accel lag at 4000 rpm/s over 12 deg ~= 0.003 deg. Secondary: localize the 1 ms tails via trgDecode vs trgPostDecode (m74_9 gap-validation code vs spark/fuel listeners), and note the executor preempts the handoff (WDA feed ~100 us every ~105 ms) adding up to 100 us to the in-flight tooth.
...
### Angle-clock on TMR2 (2026-08-29): the implementable stock-style scheme - no output-pin AF needed

Correction to the "hardware is blocked" conclusion: a GTM-style OC-on-output-pin scheme IS blocked, but the stock (Cortex-M dump I865LB52, no GTM exists on this MCU) must use the other mechanism - and that one is fully implementable on the m74_9 pins: an angle-clock timer whose OC channels work as PURE SOFTWARE COMPARATORS (CCxE=0, CCxIF interrupt fires at CNT==CCR without touching a pin), the ISR writes the GPIO edge (PF12-15 / PE8-11 via L9779). No AF needed on the output pins. Dump hint: the TMR2 base 0x40000000 appears 68x in the stock dump vs single digits for all other timers - TMR2 (32-bit, 4-ch) is exactly the right timer for an angle clock with compare events (32-bit so the 60-2 gap fits at cranking). To CONFIRM the stock's scheme: Ghidra the TMR2 references (ARR reload per tooth from the crank ISR, CCR writes from the angle queue).

Design (rusEFI, m74_9):
- Angle clock = TMR2: 32-bit, free (GPT/PWM/ICU all FALSE), IRQ 28 = VectorB0 wired in the port, TIMCLK1 = 288 MHz, PSC=71 -> 4 MHz ticks (0.25 us, NT-domain) -> 0.0105 deg/tick @7000 rpm.
- Tooth reset in the FAST EXTI ISR (prio 0, the one that timestamps edges): TMR2->CNT = 0; TMR2->ARR = predictedTicks (smoothed period x 4, written by the handoff; raw delta fallback). Constant 2-3 us offset from the edge = correctable phase constant. Hardware reset is unavailable: PF8 has only TMR13_CH1 and TMR13->TMR2 TRGO/ITR chaining does not exist on the F4 layout. The 60-2 gap needs no special handling - the edge-to-edge interval IS the gap (no events live there).
- Events: 4 OC channels, CCxS=0/CCxE=0/CCxIE=1, values in ticks from the current tooth start. 16 events per 720 deg (4-cyl) = one per ~7 teeth, so 4 channels suffice; a fired channel re-arms for the next event of the same tooth.
- TMR2 ISR at priority 3 (executor-level): SR -> which channel -> run the SAME actions (turnSparkPinHighStartCharging / fireSparkAndPrepareNextSchedule / turnInjectionPinHigh/Low) -> re-arm. ~1 us entry = fixed edge latency.
- Programming ONE TOOTH AHEAD: on tooth N the handoff converts tooth N+1's queued angle events to ticks (fraction x predicted ARR) and writes CCR. The decode chain leaves the critical path - the slack is a full tooth (166 us @6000) plus the remainder to the event.
- Fallback: events in the CURRENT tooth (post-resync, tails) stay on the TIM5 time path; multispark (time-based) stays on TIM5; TIM5 keeps serving WDA/soft-PWM/TS. Counters: "programmed late" + angle-clock vs fallback event counts for lockstats.
- HONEST LIMIT: the angle clock doubles the programming slack but does NOT cure the 1 ms tails - a 6-tooth tail still misses the programming of the teeth it spans (their events fall back to TIM5 with the old lateness). Sequence: lockstats measurement + tail localization (trgDecode/trgPostDecode) FIRST, then the angle clock as the target architecture with the TIM5 fallback as the safety net.
