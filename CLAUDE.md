...
- **The drift-proof strategy (implemented 2026-08-24, commit d6b319ce95c)**: RESPTIME=10 -> response time 15.8 ms, cycle 28.4 ms, drift +-0.8 ms (8x margin vs the 12.6 ms window). Two executor events per cycle: prepare (REQUHI/REQULO reads + period adaptation + RESP_BYTE3..1 early, ~15 ms slack) and BYTE0 (single critical frame at window center). Period = wd_delay_ms in [17, 27] ms, 5 ms adaptation steps. The BYTE0 timing does not follow the prepare's execution jitter (scheduled at prepare_sched_time + WDA_BYTE0_LEAD_MS=5).
