...
**VR AMPLITUDE MODEL (m74_9_vr_model.cpp, 2026-08-24) - rpm -> hysteresis level -> expected shift/eat-risk:** Vp = k*rpm (k = 'vrk <mV/rpm>' console command, RAM-only, 0 = off) quantized against PV 930/1600/2300/3000 mV -> level 1..5 -> HI 50/100/173/322/510 mV @10k. The expected gap shift per level is a 5-entry table SEEDED 0.63 pitch (coast measurement); the per-level split needs the 2000-7000 rpm captures. Eat-risk = Vp proximity to the nearest PV boundary (15% band) - the quantization-boundary hypothesis for the 1763-4005 band. The model feeds `custom_board_vrGapShiftPitch`: the anchor correction falls back to the model estimate when the per-sync gap measurement is out of band (stretched gap) instead of freezing. Persisting k as a tune field is the follow-up. Calibrating k: one bench VR-peak scope measurement at a known rpm, or fit from the measured shift.
