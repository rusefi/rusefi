...
- Bus arbitration: the thread's SPI batches (chip_init/update_output/vrs ramp/diag) each run under `chibios_rt::CriticalSectionLocker`, so the executor can not preempt mid-batch; the executor ISR preempts threads only outside those sections. `chip_reset()`'s `chThdSleepMilliseconds(3)` is NOT under any lock (called only from `init()` in adder context).
- The diag refresh is CHUNKED (`refresh_diag_cache(maxRegs)` + `diag_next_reg`/`diag_pending` cursor, 3 registers per 7 ms thread pass) to bound the critical section (~72 us) - the executor (and tooth-timestamp fast-IRQ) must not be blocked for the whole ~20-frame batch.
