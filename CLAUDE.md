...
- **Edge polarity (measured 2026-08-20)**: the L9779 VRS zero crossing lands on the **FALLING edge of PF8** (sensor -> L9779 VRS -> OUT_VRS -> inverting 74HC14 -> PF8). rawtrg variance analysis: F-anchored tooth periods 6.0% detrended std vs 2.6% for R-anchored - the rising edge is the hysteresis-return edge and moves with signal amplitude. Tune must use `invertPrimaryTriggerSignal = Falling`; the F->R split is a constant 38% of the tooth, so switching edges shifts the sync reference ~2.3 deg (offset 114 -> ~112, verify with a timing light). Analysis: `.tmp_edge_analysis.py` (rawtrg dump convention: `rXXX[i]` = type of the edge STARTING delta `dXXX[i]`).
