...
- Fallback: current-tooth events (post-resync, tails, no free channel, armed-too-late) stay on the TIM5 time path; multispark (time-based) stays on TIM5; TIM5 keeps serving WDA/soft-PWM/TS. Currently only the TriggerScheduler queue (spark fire + VVT/aux) gets the angle clock - dwell start and injection start are still edge-based TIM5 schedules at their due tooth (follow-up).
