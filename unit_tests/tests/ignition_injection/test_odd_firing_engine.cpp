...
//	ASSERT_NEAR(32, eth.timeToAngle(10.66666666), EPS3D);
	...
	eth.assertEvent5("spark down#3", 3, (void*)fireSparkAndPrepareNextSchedule, USOF(static_cast<efitimeus_t::rep>(eth.angleToTimeUs(expectedAngle3))));

	angle_t expectedAngle7 = 180 + cylinderOne - timing;
	eth.assertEvent5("spark down#7", 7, (void*)fireSparkAndPrepareNextSchedule, USOF(static_cast<efitimeus_t::rep>(eth.angleToTimeUs(expectedAngle7))));

	eth.assertRpm( 500, "spinning-RPM#1");

	engine->executor.executeAll(static_cast<efitick_t>(COUNTOF(getTimeNowUs()) + MS2US(1000000)));
	...
	ASSERT_EQ( 8,  engine->executor.size());
	eth.assertEvent5("fuel down2#1", 1, (void*)turnInjectionPinLow, USOF(static_cast<efitimeus_t::rep>(eth.angleToTimeUs(180 + PORT_INJECTION_OFFSET + cylinderOne))));
	eth.assertEvent5("spark down2#3", 3, (void*)fireSparkAndPrepareNextSchedule, USOF(static_cast<efitimeus_t::rep>(eth.angleToTimeUs(-180 + cylinderTwo - timing))));
	eth.assertEvent5("fuel down2#6", 6, (void*)turnInjectionPinLow, USOF(static_cast<efitimeus_t::rep>(eth.angleToTimeUs(540 + PORT_INJECTION_OFFSET + cylinderTwo))));
	eth.assertEvent5("spark down2#7", 7, (void*)fireSparkAndPrepareNextSchedule, USOF(static_cast<efitimeus_t::rep>(eth.angleToTimeUs(180 + cylinderOne - timing))));
