...
TEST(idle_v2, runningOpenLoopTpsTaperWithDashpot) {
	...
	EXPECT_FLOAT_EQ(50, dut.getRunningOpenLoop(IIdleController::Phase::Running, 0, 0, 100));
    ...
    EXPECT_FLOAT_EQ(50, dut.getRunningOpenLoop(IIdleController::Phase::Idling, 0, 0, 0));
    // now we're in the middle of decay
    timeNowUs += 5'000'000;
    // 50% decay (50% of 50 is 25)
    EXPECT_FLOAT_EQ(25, dut.getRunningOpenLoop(IIdleController::Phase::Idling, 0, 0, 0));
    // now the decay is finished
    timeNowUs += 5'000'000;
    // no correction
    EXPECT_FLOAT_EQ(0, dut.getRunningOpenLoop(IIdleController::Phase::Idling, 0, 0, 0));
    // still react to the pedal
    EXPECT_FLOAT_EQ(50, dut.getRunningOpenLoop(IIdleController::Phase::Idling, 0, 0, 10));
}
