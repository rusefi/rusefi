...
#define TEST_REVOLUTIONS 6
	...
	ASSERT_EQ(711360, engine->tdcScheduler[0].getMomentUs() % SIMULATION_CYCLE_PERIOD); // let's assert TDC position and sync point
	ASSERT_EQ(711360, engine->tdcScheduler[1].getMomentUs() % SIMULATION_CYCLE_PERIOD); // let's assert TDC position and sync point
}
