...
static void test(int engineSyncCam, float camOffsetAdd, int wc) {
	...
	// TODO: why warnings?
	ASSERT_EQ(wc, eth.recentWarnings()->getCount());
	if (wc == 2) {
	...
	ASSERT_EQ(ObdCode::CUSTOM_PRIMARY_TOO_MANY_TEETH, eth.recentWarnings()->get(wc - 1).Code);
}
